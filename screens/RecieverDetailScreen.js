...
export default class RecieverDetailScreen extends  React.Component{
    ...
    getRecieverDetails=()=>{
        db.collection('users').where('email_id','==',this.state.recieverId)
        .get().then((snapshot)=>{
    ...
    render(){
        return(
            <View style={styles.container}>
                <View style={{flex:0.1}}>
                    <Header leftComponent={<Icon name='arrow_left' type='feather' color='blue' onPress={()=>{
                        this.props.navigation.goBack()
                    }}/>}
                    centerComponent={{text:'Donate Books' , style:{color:'yellow',fontSize:20,fontWeight:'bold'}}}
                    backgroundColor='red'
                    />
                </View>
                <View style={{flex:0.3}}>
